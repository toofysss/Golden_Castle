...
if (isset($_POST['user']) && isset($_POST['firstname']) && isset($_POST['lastname']) && isset($_POST['phonenumber'])) {
    ...
    $image = imageUpload('image', "../../$locationImg/imguser/");
    $query = "SELECT email FROM T_Conference_User WHERE email = ?";
    $params = array($user);
    $result = sqlsrv_query($conn, $query, $params);
    $check_email = sqlsrv_has_rows($result);
    if ($check_email) {
        echo json_encode(array("status" => "faild", "data" => "EmailFound"));
        exit();
    } else {
        $query = "INSERT INTO T_Conference_User (FullName, email, PhoneNum, ImageUser, pass) VALUES (?, ?, ?, ?, ?)";
        $stmt = sqlsrv_query($conn, $query, array($FullName, $user, $phonenumber, $image, $hashedPassword));
        try {
            $mail = new PHPMailer(true);

            require "../../Data/email.php";

            $mail->addAddress($user, 'Admin Name');
                                              ...
                                              <tr>
                                                <td class="pad">
                                                  <div
                                                    ...
                                                    "
                                                  >
                                                    <p style="margin: 0">انت الان مشترك في </p>
                                                    <p style="margin: 0"> .   منصة العراق الاقتصادية</p>
                                          ...
                                          <tr>
                                            <td class="pad">
                                              <div
                                                ...
                                                "
                                              >
                                                <p style="margin: 0">ان كلمة السر  الخاصة بك هي</p>
                                                <input type="text" value=' . $userPassword . ' style="margin: 0; height:55px; width: 450px; border-radius: 15px; text-align: center; font-size: 28px;" disabled/>
